...
public class FiltroLogin implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)      
        ...
        if(controller && req.getParameter("op").equals(Variabili.LOGIN)){
            chain.doFilter(req, resp);
        }
        else if(controller && req.getParameter("op").equals(Variabili.GRUPPIPUBBLICI)){
            chain.doFilter(req, resp);
        }
        else if(controller && req.getParameter("op").equals(Variabili.REGISTRAZIONE)){
            chain.doFilter(req, resp);
        }
        else if(controller && req.getParameter("op").equals(Variabili.RECUPERO_PASS)){
            chain.doFilter(req, resp);
        }
        else if(req.getRequestURI().equals(recuperoPassURL)){
            chain.doFilter(req, resp);
        }
        else if(req.getRequestURI().equals(registrazioneURL)){
            chain.doFilter(req, resp);
        }
        else if (session.getAttribute("user") == null && !req.getRequestURI().equals(loginURL)) {
            resp.sendRedirect(loginURL);
        }
        else if (session.getAttribute("user") != null && req.getRequestURI().equals(loginURL)) {
            resp.sendRedirect("/Controller?op=home");
        }
        else {
            chain.doFilter(req, resp);
        }
